resource "aws_apigatewayv2_api" "invoice_api" {
  name          = "invoice-api"
  protocol_type = "HTTP"

  tags = {
    Project     = "Secure Serverless Invoice System"
    Environment = "Development"
  }
}

resource "aws_apigatewayv2_integration" "invoice_lambda" {
  api_id = aws_apigatewayv2_api.invoice_api.id

  integration_type = "AWS_PROXY"

  integration_uri = aws_lambda_function.invoice_processor.invoke_arn

  payload_format_version = "2.0"
}

resource "aws_apigatewayv2_route" "upload_invoice" {
  api_id = aws_apigatewayv2_api.invoice_api.id

  route_key = "POST /invoice"

  target = "integrations/${aws_apigatewayv2_integration.invoice_lambda.id}"
}

resource "aws_apigatewayv2_stage" "default" {
  api_id = aws_apigatewayv2_api.invoice_api.id

  name = "$default"

  auto_deploy = true
}

resource "aws_lambda_permission" "allow_apigateway" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.invoice_processor.function_name
  principal     = "apigateway.amazonaws.com"

  source_arn = "${aws_apigatewayv2_api.invoice_api.execution_arn}/*/*"
}